...
if [ ! -f "$DEPLOYMENT_MARKER_FILE" ]; then
    ...
    if [[ "${CREDENZA_ISOLATION_ENABLED}" == "true" ]]; then
      if [ -f /etc/apache2/conf-enabled/wsgi_credenza.conf ]; then
        mv /etc/apache2/conf-enabled/wsgi_credenza.conf /etc/apache2/conf-available/
      fi
    else
      if [ ! -f /etc/apache2/conf-enabled/wsgi_credenza.conf ]; then
        a2enconf wsgi_credenza
      fi
    fi
...
# Suppress cert verify warnings for 'localhost' and 'deriva', generated by inter-service communication when cert verify is disabled
export PYTHONWARNINGS="ignore:Unverified HTTPS request is being made to host 'localhost'.,ignore:Unverified HTTPS request is being made to host 'deriva'"
